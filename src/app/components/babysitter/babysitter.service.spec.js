(function() {
  ...
  describe('babysitter.service', function(){
    ...
    it('should reject the promise when the start time is not a moment', inject(function() {
      var resolved, rejected;

      babysitterService.calculateCharge(Date.now()).then(
        function (value) {
          resolved = value;
        },
        function (value) {
    ...
    it('should reject the promise when the bed time is not a moment', inject(function() {
      var resolved, rejected;

      babysitterService.calculateCharge(validStart, validFinishSameDay, Date.now()).then(
        function (value) {
          resolved = value;
        },
        function (value) {
      ...
      expect(resolved).toBeUndefined();
      expect(rejected).toBe('The bed time is not a Moment');
    }));
  });
})();
